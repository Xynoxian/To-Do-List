...
from task_manager import create_task, get_tasks, mark_task_status, add_subtask, get_subtasks, toggle_subtask, delete_subtask, get_task_history, get_categories, get_upcoming_tasks, check_recurring_tasks
from analytics import get_completion_stats, get_on_time_rate, get_productivity_score, get_streak, get_tasks_by_priority, get_tasks_by_category, check_achievements, get_achievements
from scheduler import calculate_priority_score, get_smart_task_order, detect_deadline_conflicts, get_workload_summary, predict_delayed_tasks
...
uid = user['id']
now = datetime.now()
tasks_data = [('Meeting prep', 'Work', 'High', 2), ('Exercise', 'Health', 'Medium', 24), ('Read book', 'Personal', 'Low', 72), ('Submit assignment', 'Study', 'Urgent', 1), ('Clean house', 'Personal', 'Medium', 48), ('Team standup', 'Work', 'High', 3), ('Buy birthday gift', 'Personal', 'Medium', 5), ('Fix bug #42', 'Work', 'Urgent', 0.5)]
task_ids = []
...
rec_id, _ = create_task(uid, 'Daily standup', '', 'Work', 'Medium', now.strftime('%Y-%m-%d %H:%M:%S'), is_recurring=1, recurrence_pattern='daily')
...
mark_task_status(task_ids[1], uid, 'Done')
mark_task_status(task_ids[2], uid, 'Done')
mark_task_status(task_ids[4], uid, 'Done')
...
test('Urgent near-deadline scores higher than low far-deadline', score_urgent > score_low)
...
test('Sorted by score descending', all((smart_order[i]['ai_score'] >= smart_order[i + 1]['ai_score'] for i in range(len(smart_order) - 1))))
conflict_date = (now + timedelta(days=2)).strftime('%Y-%m-%d')
for i in range(4):
    create_task(uid, f'Conflict task {i + 1}', '', 'Test', 'Medium', f'{conflict_date} {10 + i}:00:00')

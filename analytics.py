"""
analytics.py — Productivity analytics, achievement monitoring,
and performance tracking for the To-Do List Organizer.
"""
...
def get_tasks_per_day(user_id, days=14):
    """Task creation/completion counts per day for last N days."""
    conn = get_connection()
    results = []
...
def get_avg_completion_time(user_id):
    """Average hours between creation and completion."""
    conn = get_connection()
...
def get_productivity_score(user_id):
    """0-100 productivity score: completion(40%) + on-time(30%) + health(30%)."""
...
ACHIEVEMENT_DEFS = [
    ("First Task", "Create your first task", lambda s, u: s["total"] >= 1),
    ("Getting Started", "Complete your first task", lambda s, u: s["done"] >= 1),
    ("Task Master", "Complete 10 tasks", lambda s, u: s["done"] >= 10),
    ("Prolific", "Complete 25 tasks", lambda s, u: s["done"] >= 25),
    ("On Fire", "3-day completion streak", lambda s, u: get_streak(u) >= 3),
    ("Week Warrior", "7-day completion streak", lambda s, u: get_streak(u) >= 7),
    ("Organized", "Use 3+ categories", lambda s, u: _count_categories(u) >= 3),
    ("Productivity Pro", "Score 80+ productivity", lambda s, u: get_productivity_score(u) >= 80),
]


def check_achievements(user_id):
    """Award new achievements. Returns list of newly earned."""
    stats = get_completion_stats(user_id)
    conn = get_connection()

"""
analytics_frame.py — Productivity analytics dashboard with charts,
stats cards, and achievement badges.
"""

import customtkinter as ctk
from gui.styles import COLORS, FONTS
from analytics import (get_completion_stats, get_on_time_rate,
                       get_productivity_score, get_streak, get_avg_completion_time,
                       get_tasks_per_day, get_tasks_by_priority, get_tasks_by_status,
                       get_tasks_by_category, get_achievements, check_achievements)
...
class AnalyticsFrame(ctk.CTkFrame):
    """Analytics dashboard with stats, charts, and achievements."""
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(scroll, text="📈 Analytics & Insights", font=FONTS["heading"],
        ...
        card_data = [
            ("Total Tasks", str(stats["total"]), COLORS["accent"]),
            ("Completed", str(stats["done"]), COLORS["success"]),
            ("Completion Rate", f"{stats['completion_rate']}%", COLORS["accent"]),
            ("On-Time Rate", f"{on_time}%", COLORS["success"]),
            ("Productivity", f"{score}/100", COLORS["warning"] if score < 70 else COLORS["success"]),
            ("Streak", f"{streak} day{'s' if streak != 1 else ''}", COLORS["accent"]),
        ]
        ...
        for i, (label, val, color) in enumerate(card_data):
            ...
            ctk.CTkLabel(c, text=val, font=("Segoe UI", 22, "bold"),
                         text_color=color).pack(padx=12, pady=(12, 2))
            ctk.CTkLabel(c, text=label, font=FONTS["tiny"],
                         text_color=COLORS["text_secondary"]).pack(padx=12, pady=(0, 10))
        ...
        if HAS_MPL:
            ...
            ctk.CTkLabel(trend_card, text="14-Day Activity Trend", font=FONTS["body_bold"],
                         text_color=COLORS["text_primary"]).pack(padx=15, pady=(10, 5))
            ...
            ctk.CTkLabel(pie_card, text="By Priority", font=FONTS["body_bold"],
                         text_color=COLORS["text_primary"]).pack(padx=15, pady=(10, 5))
            self._draw_priority_pie(pie_card)

        # AI Insights section
        ctk.CTkLabel(scroll, text="🤖 AI Insights", font=FONTS["subheading"],
        ...
        ctk.CTkLabel(wl_inner, text="Workload Overview", font=FONTS["body_bold"],
        ...
        ctk.CTkLabel(wl_inner, text=wl_stats, font=FONTS["small"],
                     text_color=COLORS["text_secondary"]).pack(anchor="w", pady=(2, 6))

        for sug in workload["suggestions"]:
            ctk.CTkLabel(wl_inner, text=sug, font=FONTS["small"],
                         text_color=COLORS["text_primary"], anchor="w",
                         wraplength=700).pack(fill="x", pady=1)

        # Deadline conflicts
        conflicts = detect_deadline_conflicts(self.user_id)
        if conflicts:
            ctk.CTkLabel(wl_inner, text="", font=FONTS["tiny"]).pack()
            ctk.CTkLabel(wl_inner, text="⚠️ Deadline Conflicts", font=FONTS["body_bold"],
                         text_color=COLORS["warning"]).pack(anchor="w")
            for c in conflicts[:5]:
                ctk.CTkLabel(wl_inner, text=c["message"], font=FONTS["small"],
        ...
        if at_risk:
            ctk.CTkLabel(wl_inner, text="", font=FONTS["tiny"]).pack()
            ctk.CTkLabel(wl_inner, text="🚨 At-Risk Tasks", font=FONTS["body_bold"],
                         text_color=COLORS["danger"]).pack(anchor="w")
            for r in at_risk[:5]:
                risk_color = COLORS["danger"] if r["risk"] == "High" else COLORS["warning"]
                ctk.CTkLabel(wl_inner, text=f"• [{r['risk']}] {r['title']}: {r['reason']}",
                             font=FONTS["small"], text_color=risk_color,
                             wraplength=700).pack(anchor="w", pady=1)

        # Smart Task Order
        smart = get_smart_task_order(self.user_id)
        if smart:
            ctk.CTkLabel(scroll, text="🎯 Suggested Task Order", font=FONTS["subheading"],
                         text_color=COLORS["text_primary"]).pack(anchor="w", pady=(10, 8))
            ...
            for i, t in enumerate(smart[:8]):
                ...
                ctk.CTkLabel(row, text=f"#{i+1}", font=FONTS["body_bold"],
                             text_color=COLORS["accent"], width=35).pack(side="left")
                ctk.CTkLabel(row, text=t["title"], font=FONTS["body"],
                             text_color=COLORS["text_primary"]).pack(side="left", padx=5)
                ctk.CTkLabel(row, text=f"Score: {t['ai_score']}", font=FONTS["tiny"],
                             text_color=COLORS["text_secondary"]).pack(side="right")

        # Achievements
        achievements = get_achievements(self.user_id)
        ctk.CTkLabel(scroll, text="🏆 Achievements", font=FONTS["subheading"],
                     text_color=COLORS["text_primary"]).pack(anchor="w", pady=(10, 8))
        ...
        if achievements:
            for a in achievements:
                ...
                ctk.CTkLabel(row, text=a["achievement_name"], font=FONTS["body_bold"],
                             text_color=COLORS["text_primary"]).pack(side="left", padx=8)
                ctk.CTkLabel(row, text=a.get("description", ""), font=FONTS["small"],
                             text_color=COLORS["text_secondary"]).pack(side="left")
        else:
            ctk.CTkLabel(ach_frame, text="  Complete tasks to earn achievements!",
                         font=FONTS["small"],
    ...
    def _draw_trend_chart(self, parent):
        ...
        ax.bar([i - 0.15 for i in range(len(dates))], created, 0.3,
               label="Created", color=COLORS["accent"], alpha=0.8)
        ax.bar([i + 0.15 for i in range(len(dates))], completed, 0.3,
               label="Completed", color=COLORS["success"], alpha=0.8)
    ...
    def _draw_priority_pie(self, parent):
        ...
        pie_colors = [COLORS["priority_low"], COLORS["priority_medium"],
                      COLORS["priority_high"], COLORS["priority_urgent"]]

        if sum(values) == 0:
            ctk.CTkLabel(parent, text="No tasks yet", font=FONTS["small"],

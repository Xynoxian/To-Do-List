...
KANBAN_COLUMNS = [('Pending', '⏳'), ('In Progress', '🔄'), ('Done', '✅'), ('Late', '⏰')]
...
class KanbanFrame(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header, text='📊 Kanban Board', font=FONTS['heading'], text_color=COLORS['text_primary']).pack(side='left')
    ...
    def _build_column(self, status, emoji, tasks):
        ...
        col = ctk.CTkFrame(self.columns_frame, fg_color=COLORS['bg_card'], corner_radius=12)
        header = ctk.CTkFrame(col, fg_color='transparent')
        header.pack(fill='x', padx=12, pady=(12, 6))
        ctk.CTkLabel(header, text=f'{emoji} {status}', font=FONTS['body_bold'], text_color=col_color).pack(side='left')
        ctk.CTkLabel(header, text=str(len(tasks)), font=FONTS['small'], text_color=COLORS['text_secondary'], fg_color=COLORS['bg_secondary'], corner_radius=10, width=28, height=22).pack(side='right')
    ...
    def _render_kanban_card(self, parent, task, current_status):
        ...
        ctk.CTkLabel(inner, text=f'{pri_emoji} {task['title']}', font=FONTS['small'], text_color=COLORS['text_primary'], anchor='w', wraplength=180).pack(fill='x')
        if task.get('due_date'):
            ctk.CTkLabel(inner, text=f'📅 {task['due_date'][:10]}', font=FONTS['tiny'], text_color=COLORS['text_secondary']).pack(anchor='w')
        move_frame = ctk.CTkFrame(inner, fg_color='transparent')
        move_frame.pack(fill='x', pady=(4, 0))
        moves = {'Pending': [('▶', 'In Progress'), ('✅', 'Done')], 'In Progress': [('◀', 'Pending'), ('✅', 'Done')], 'Done': [('◀', 'Pending')], 'Late': [('▶', 'In Progress'), ('✅', 'Done')]}
        for btn_text, target in moves.get(current_status, []):
            ctk.CTkButton(move_frame, text=btn_text, width=30, height=24, font=FONTS['tiny'], fg_color=COLORS['bg_tertiary'], hover_color=COLORS['accent'], command=lambda tid=task['id'], s=target: self._move(tid, s)).pack(side='left', padx=1)
        ctk.CTkButton(move_frame, text='✏️', width=30, height=24, font=FONTS['tiny'], fg_color=COLORS['bg_tertiary'], hover_color=COLORS['accent'], command=lambda tid=task['id']: self._edit(tid)).pack(side='right')

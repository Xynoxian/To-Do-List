"""
login_frame.py — Login and Registration screens with input validation.
"""
...
class LoginFrame(ctk.CTkFrame):
    """Full-screen login / registration frame."""
    ...
    def _build_ui(self):
        ...
        center = ctk.CTkFrame(self, fg_color=COLORS["bg_card"],
                              corner_radius=20, width=420, height=600)
        ...
        # App title
        ctk.CTkLabel(inner, text="✅ TaskFlow",
                     font=("Segoe UI", 28, "bold"),
                     text_color=COLORS["accent"]).pack(pady=(10, 2))
        ctk.CTkLabel(inner, text="Smart To-Do Manager",
                     font=FONTS["small"],
                     text_color=COLORS["text_secondary"]).pack(pady=(0, 20))

        # Mode toggle
        toggle_frame = ctk.CTkFrame(inner, fg_color=COLORS["bg_secondary"],
                                    corner_radius=10)
        toggle_frame.pack(fill="x", pady=(0, 15))
        self.login_btn = ctk.CTkButton(
            toggle_frame, text="Sign In", font=FONTS["button"],
            fg_color=COLORS["accent"], hover_color=COLORS["accent_hover"],
            corner_radius=8, command=lambda: self._switch_mode(True))
        self.login_btn.pack(side="left", expand=True, fill="x", padx=3, pady=3)
        self.register_btn = ctk.CTkButton(
            toggle_frame, text="Register", font=FONTS["button"],
            fg_color="transparent", hover_color=COLORS["bg_tertiary"],
            text_color=COLORS["text_secondary"], corner_radius=8,
            command=lambda: self._switch_mode(False))
    ...
    def _switch_mode(self, is_login):
        ...
        if is_login:
            self.login_btn.configure(fg_color=COLORS["accent"],
                                     text_color=COLORS["text_primary"])
            self.register_btn.configure(fg_color="transparent",
                                       text_color=COLORS["text_secondary"])
            self._build_login_form()
        else:
            self.register_btn.configure(fg_color=COLORS["accent"],
                                       text_color=COLORS["text_primary"])
    ...
    def _build_login_form(self):
        self._clear_form()
        ctk.CTkLabel(self.form, text="Username", font=FONTS["small"],
                     text_color=COLORS["text_secondary"], anchor="w"
                     ).pack(fill="x", pady=(5, 2))
        self.login_user_entry = ctk.CTkEntry(
            self.form, font=FONTS["body"], height=40,
            fg_color=COLORS["bg_secondary"], border_color=COLORS["border"],
            placeholder_text="Enter your username")
        self.login_user_entry.pack(fill="x", pady=(0, 8))

        ctk.CTkLabel(self.form, text="Password", font=FONTS["small"],
                     text_color=COLORS["text_secondary"], anchor="w"
                     ).pack(fill="x", pady=(0, 2))
        self.login_pass_entry = ctk.CTkEntry(
            self.form, font=FONTS["body"], height=40, show="•",
            fg_color=COLORS["bg_secondary"], border_color=COLORS["border"],
            placeholder_text="Enter your password")
        self.login_pass_entry.pack(fill="x", pady=(0, 15))
        self.login_pass_entry.bind("<Return>", lambda e: self._do_login())

        self.login_msg = ctk.CTkLabel(self.form, text="", font=FONTS["small"],
                                      text_color=COLORS["danger"])
        self.login_msg.pack(pady=(0, 5))

        ctk.CTkButton(self.form, text="Sign In", font=FONTS["button"],
                      height=42, fg_color=COLORS["accent"],
                      hover_color=COLORS["accent_hover"], corner_radius=10,
                      command=self._do_login).pack(fill="x")

    def _build_register_form(self):
        self._clear_form()
        fields = [("Username", "Choose a username", False),
                  ("Email", "your@email.com", False),
                  ("Password", "At least 6 characters", True),
                  ("Confirm Password", "Re-enter password", True)]
        self.reg_entries = {}
        for label, ph, is_pw in fields:
            ctk.CTkLabel(self.form, text=label, font=FONTS["small"],
                         text_color=COLORS["text_secondary"], anchor="w"
                         ).pack(fill="x", pady=(3, 1))
            e = ctk.CTkEntry(self.form, font=FONTS["body"], height=38,
        ...
        self.reg_msg = ctk.CTkLabel(self.form, text="", font=FONTS["small"],
                                    text_color=COLORS["danger"])
        self.reg_msg.pack(pady=(3, 5))

        ctk.CTkButton(self.form, text="Create Account", font=FONTS["button"],
                      height=42, fg_color=COLORS["success"],
                      hover_color="#2ea043", corner_radius=10,
                      command=self._do_register).pack(fill="x")

    def _do_login(self):
        user, msg = login_user(self.login_user_entry.get(),

"""
task_list.py — Filterable, sortable task list with search, status badges, and actions.
"""
...
class TaskListFrame(ctk.CTkFrame):
    """Full task list view with search, filters, and sorting."""
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header, text="📋 My Tasks", font=FONTS["heading"],
        ...
        search = ctk.CTkEntry(inner_bar, textvariable=self.search_var,
                               font=FONTS["small"], height=34, width=220,
        ...
        ctk.CTkButton(inner_bar, text="Search", font=FONTS["small"],
                      width=70, height=34, fg_color=COLORS["accent"],
                      command=self.refresh).pack(side="left", padx=(0, 15))

        # Status filter
        ctk.CTkLabel(inner_bar, text="Status:", font=FONTS["small"],
                     text_color=COLORS["text_secondary"]).pack(side="left", padx=(0, 4))
        self.status_var = ctk.StringVar(value="All")
        ctk.CTkOptionMenu(inner_bar, values=["All", "Pending", "In Progress", "Done", "Late"],
                          variable=self.status_var, font=FONTS["small"],
                          height=34, width=110, fg_color=COLORS["bg_secondary"],
                          button_color=COLORS["accent"],
                          command=lambda _: self.refresh()).pack(side="left", padx=(0, 10))

        # Priority filter
        ctk.CTkLabel(inner_bar, text="Priority:", font=FONTS["small"],
                     text_color=COLORS["text_secondary"]).pack(side="left", padx=(0, 4))
        self.priority_var = ctk.StringVar(value="All")
        ctk.CTkOptionMenu(inner_bar, values=["All", "Low", "Medium", "High", "Urgent"],
                          variable=self.priority_var, font=FONTS["small"],
                          height=34, width=100, fg_color=COLORS["bg_secondary"],
                          button_color=COLORS["accent"],
                          command=lambda _: self.refresh()).pack(side="left", padx=(0, 10))

        # Sort
        ctk.CTkLabel(inner_bar, text="Sort:", font=FONTS["small"],
                     text_color=COLORS["text_secondary"]).pack(side="left", padx=(0, 4))
        self.sort_var = ctk.StringVar(value="due_date")
        ctk.CTkOptionMenu(inner_bar, values=["due_date", "priority", "created_at", "title"],
                          variable=self.sort_var, font=FONTS["small"],
                          height=34, width=110, fg_color=COLORS["bg_secondary"],
                          button_color=COLORS["accent"],
    ...
    def refresh(self):
        ...
        tasks = get_tasks(self.user_id, status=status, priority=pri,
                          search=search, sort_by=self.sort_var.get())

        if not tasks:
            ctk.CTkLabel(self.task_scroll, text="No tasks found. Create one!",
    ...
    def _render_task_card(self, task):
        ...
        card = ctk.CTkFrame(self.task_scroll, fg_color=COLORS["bg_card"],
        ...
        # Left: priority indicator + title + description
        ...
        ctk.CTkLabel(title_row, text=title_text, font=FONTS["body_bold"],
                     text_color=COLORS["text_primary"], anchor="w").pack(side="left")

        # Status badge
        stat_emoji = STATUS_EMOJI.get(task["status"], "")
        badge = ctk.CTkLabel(title_row, text=f" {stat_emoji} {task['status']} ",
                             font=FONTS["tiny"], text_color=stat_color,
                             fg_color=COLORS["bg_secondary"], corner_radius=5)
        badge.pack(side="left", padx=8)

        # Category badge
        if task.get("category") and task["category"] != "General":
            ctk.CTkLabel(title_row, text=f" {task['category']} ",
                         font=FONTS["tiny"], text_color=COLORS["accent"],
                         fg_color=COLORS["bg_secondary"],
                         corner_radius=5).pack(side="left", padx=2)

        # Recurring badge
        if task.get("is_recurring"):
            ctk.CTkLabel(title_row, text=f" 🔁 {task.get('recurrence_pattern', '')} ",
        ...
        if desc:
            ...
            ctk.CTkLabel(left, text=preview, font=FONTS["small"],
        ...
        if task.get("due_date"):
            ...
            ctk.CTkLabel(left, text=due_text, font=FONTS["tiny"],
                         text_color=COLORS["text_secondary"],
                         anchor="w").pack(fill="x", pady=(2, 0))
        ...
        if subs:
            ...
            ctk.CTkLabel(left, text=f"  ☑ {done_count}/{len(subs)} subtasks",
                         font=FONTS["tiny"],
                         text_color=COLORS["text_secondary"],
        ...
        if task["status"] != "Done":
            ctk.CTkButton(actions, text="✅", width=32, height=32, font=("Segoe UI", 14),
                          fg_color=COLORS["success"], hover_color="#2ea043",
                          command=lambda t=task: self._set_status(t["id"], "Done")
                          ).pack(side="left", padx=2)

        if task["status"] not in ("Late", "Done"):
            ctk.CTkButton(actions, text="⏰", width=32, height=32, font=("Segoe UI", 14),
                          fg_color=COLORS["warning"], hover_color="#b8860b",
                          command=lambda t=task: self._set_status(t["id"], "Late")
                          ).pack(side="left", padx=2)

        if task["status"] == "Pending":
            ctk.CTkButton(actions, text="▶", width=32, height=32, font=("Segoe UI", 14),
                          fg_color=COLORS["accent"], hover_color=COLORS["accent_hover"],
                          command=lambda t=task: self._set_status(t["id"], "In Progress")
                          ).pack(side="left", padx=2)

        ctk.CTkButton(actions, text="✏️", width=32, height=32, font=("Segoe UI", 14),
                      fg_color=COLORS["bg_tertiary"], hover_color=COLORS["bg_secondary"],
                      command=lambda t=task: self._edit_task(t["id"])
                      ).pack(side="left", padx=2)

        ctk.CTkButton(actions, text="🗑️", width=32, height=32, font=("Segoe UI", 14),
                      fg_color=COLORS["danger"], hover_color="#b22d2d",
    ...
    def _delete_task(self, task_id):
        ...
        dialog.grab_set()
        ctk.CTkLabel(dialog, text="Delete this task?", font=FONTS["body_bold"],
                     text_color=COLORS["text_primary"]).pack(pady=(25, 15))
        bf = ctk.CTkFrame(dialog, fg_color="transparent")
        bf.pack()
        ctk.CTkButton(bf, text="Cancel", fg_color=COLORS["bg_tertiary"],
                      command=dialog.destroy).pack(side="left", padx=10)
        ctk.CTkButton(bf, text="Delete", fg_color=COLORS["danger"],
                      command=lambda: (delete_task(task_id, self.user_id),

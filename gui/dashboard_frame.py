...
from task_manager import get_tasks, get_upcoming_tasks, get_overdue_tasks, mark_task_status, create_task, check_recurring_tasks
...
class DashboardFrame(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(scroll, text=f'{greeting}, {name}! 👋', font=FONTS['heading'], text_color=COLORS['text_primary']).pack(anchor='w', pady=(0, 5))
        ...
        self.quick_entry = ctk.CTkEntry(q_inner, font=FONTS['body'], height=38, fg_color=COLORS['bg_secondary'], border_color=COLORS['border'], placeholder_text='⚡ Quick add a task...')
        self.quick_entry.pack(side='left', fill='x', expand=True, padx=(0, 8))
        self.quick_entry.bind('<Return>', lambda e: self._quick_add())
        ctk.CTkButton(q_inner, text='Add', font=FONTS['button'], width=70, height=38, fg_color=COLORS['accent'], hover_color=COLORS['accent_hover'], command=self._quick_add).pack(side='right')
        ...
        card_data = [('📋 Total', str(stats['total']), COLORS['accent']), ('✅ Done', str(stats['done']), COLORS['success']), ('⏳ Pending', str(stats['pending']), COLORS['warning']), ('⏰ Overdue', str(stats['overdue']), COLORS['danger']), ('📊 Score', f'{score}', COLORS['accent']), ('🔥 Streak', f'{streak}d', COLORS['success'])]
        for i, (label, val, color) in enumerate(card_data):
            ...
            ctk.CTkLabel(c, text=val, font=('Segoe UI', 24, 'bold'), text_color=color).pack(padx=15, pady=(12, 0))
            ctk.CTkLabel(c, text=label, font=FONTS['tiny'], text_color=COLORS['text_secondary']).pack(padx=15, pady=(0, 10))
        ...
        if workload['suggestions']:
            ...
            ctk.CTkLabel(sug_frame, text='💡 Smart Suggestions', font=FONTS['body_bold'], text_color=COLORS['accent']).pack(anchor='w', padx=15, pady=(10, 5))
            for s in workload['suggestions']:
                ctk.CTkLabel(sug_frame, text=s, font=FONTS['small'], text_color=COLORS['text_primary'], wraplength=700).pack(anchor='w', padx=15, pady=2)
        ...
        ctk.CTkLabel(up_card, text='📅 Upcoming (48h)', font=FONTS['body_bold'], text_color=COLORS['accent']).pack(anchor='w', padx=15, pady=(10, 5))
        if upcoming:
            for t in upcoming[:6]:
                self._render_mini_task(up_card, t)
        else:
            ctk.CTkLabel(up_card, text='  No upcoming tasks', font=FONTS['small'], text_color=COLORS['text_secondary']).pack(padx=15, pady=8)
        ...
        ctk.CTkLabel(ov_card, text='🚨 Overdue', font=FONTS['body_bold'], text_color=COLORS['danger']).pack(anchor='w', padx=15, pady=(10, 5))
        if overdue:
            for t in overdue[:6]:
                self._render_mini_task(ov_card, t, is_overdue=True)
        else:
            ctk.CTkLabel(ov_card, text='  All caught up! 🎉', font=FONTS['small'], text_color=COLORS['success']).pack(padx=15, pady=8)
        ctk.CTkLabel(ov_card, text='', font=FONTS['tiny']).pack()
        recent = get_tasks(self.user_id, sort_by='created_at', sort_order='DESC')
        if recent:
            ctk.CTkLabel(scroll, text='🕐 Recent Tasks', font=FONTS['body_bold'], text_color=COLORS['text_primary']).pack(anchor='w', pady=(0, 5))
            for t in recent[:5]:
    ...
    def _render_mini_task(self, parent, task, is_overdue=False, show_status=False):
        ...
        ctk.CTkLabel(inner, text=title_text, font=FONTS['small'], text_color=COLORS['text_primary'], anchor='w').pack(side='left', fill='x', expand=True)
        if show_status:
            stat_emoji = STATUS_EMOJI.get(task['status'], '')
            ctk.CTkLabel(inner, text=f'{stat_emoji} {task['status']}', font=FONTS['tiny'], text_color=COLORS['text_secondary']).pack(side='right', padx=(5, 0))
        if task.get('due_date'):
            due_display = task['due_date'][:16]
            color = COLORS['danger'] if is_overdue else COLORS['text_secondary']
            ctk.CTkLabel(inner, text=due_display, font=FONTS['tiny'], text_color=color).pack(side='right')
        if task['status'] != 'Done':
            ctk.CTkButton(inner, text='✅', width=26, height=26, font=('Segoe UI', 11), fg_color=COLORS['success'], hover_color='#2ea043', command=lambda t=task: self._quick_done(t['id'])).pack(side='right', padx=(5, 0))

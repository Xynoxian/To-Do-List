"""
scheduler.py — AI-Based Intelligent Scheduling & Smart Work Planning.
Uses rule-based algorithms for task prioritization, conflict detection,
delay prediction, and workload management suggestions.
"""
...
def calculate_priority_score(task):
    """
    Score a task 0-100 based on urgency, importance, and deadline proximity.
...
def get_smart_task_order(user_id):
    """Return tasks sorted by AI priority score (highest first)."""
    conn = get_connection()
    tasks = conn.execute(
        "SELECT * FROM tasks WHERE user_id=? AND status NOT IN ('Done')",
        (user_id,)).fetchall()
    conn.close()
...
def detect_deadline_conflicts(user_id):
    """Find days with 3+ tasks due — potential overload."""
    conn = get_connection()
    tasks = conn.execute("""
    ...
    for row in tasks:
        conflicts.append({
            "date": row["due_date"][:10] if row["due_date"] else "Unknown",
            "count": row["c"],
            "message": f"You have {row['c']} tasks due on {row['due_date'][:10]}. Consider rescheduling some."
        })
    return conflicts


def predict_delayed_tasks(user_id):
    """Flag tasks likely to be late based on avg completion time."""
    avg_hours = get_avg_completion_time(user_id)
    if avg_hours <= 0:
        avg_hours = 24  # default assumption

    conn = get_connection()
    tasks = conn.execute(
        "SELECT * FROM tasks WHERE user_id=? AND status NOT IN ('Done', 'Late') AND due_date IS NOT NULL",
        (user_id,)).fetchall()
    conn.close()
...
def get_workload_summary(user_id):
    """Generate workload insights and suggestions."""
    conn = get_connection()
    now = datetime.now()
    ...
    today_count = conn.execute(
        "SELECT COUNT(*) as c FROM tasks WHERE user_id=? AND DATE(due_date)=? AND status!='Done'",
        (user_id, today)).fetchone()["c"]
    tomorrow_count = conn.execute(
        "SELECT COUNT(*) as c FROM tasks WHERE user_id=? AND DATE(due_date)=? AND status!='Done'",
        (user_id, tomorrow)).fetchone()["c"]
    week_count = conn.execute(
        "SELECT COUNT(*) as c FROM tasks WHERE user_id=? AND DATE(due_date) BETWEEN ? AND ? AND status!='Done'",
        (user_id, today, week_end)).fetchone()["c"]
    total_active = conn.execute(
        "SELECT COUNT(*) as c FROM tasks WHERE user_id=? AND status NOT IN ('Done')",
        (user_id,)).fetchone()["c"]
    conn.close()
